...
function init() {
  ...
  const getSideWindows = () => {
    const windows = rightSidebar
      .getWindows()
      .sort((a, b) => a.order - b.order) as SideWindowInfo[];

    document.querySelectorAll(".rm-sidebar-window").forEach((w, i) => {
      ...
      if (mutation.type === "childList") {
        ...
        mutation.addedNodes.forEach((node) => {
          if ((node as Element).querySelector(".rm-sidebar-window")) {
            ...
            (node as Element).className === "rm-sidebar-outline-wrapper"
          ) {
            main();
          }
        });
...
function SwitchButton(props: { onClick: () => void }) {
  return (
    <Button icon="swap-horizontal" minimal small onClick={props.onClick} />
  );
}
